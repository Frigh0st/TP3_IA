...
#include "Machine_Learning_Bot.h"
...
#include "armory/Projectile_Bombe.h"
...
//------------------------------ AddBombe --------------------------------
void Raven_Game::AddBombe(Raven_Bot* shooter, Vector2D target)
{
	Raven_Projectile* rp = new Bombe(shooter, target);

	m_Projectiles.push_back(rp);

#ifdef LOG_CREATIONAL_STUFF
	debug_con << "Adding a bombe(grenade) " << rp->ID() << " at pos " << rp->Pos() << "";
#endif
}
